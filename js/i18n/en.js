...
export default {
  ...
  'left.search': 'Search every field & lorebook...',
  ...
  'toast.langOffline': 'That language pack is not available offline yet — showing English. It will load next time you are online.',
  ...
  // Card diagnostics (see js/cardHealth.js). Values are interpolated from the
  // issue's `values` object, so the placeholders must match those keys.
  ...
  // Version history (one entry per card, in the preview modal).
  ...
  // Command palette (Ctrl/Cmd+K). `palette.jumpTo` wraps a field label, so the
  // field name is the interpolated value, not a suffix.
